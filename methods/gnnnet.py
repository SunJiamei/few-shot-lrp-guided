...
class GnnNet(MetaTemplate):
  ...
  def __init__(self, model_func,  n_way, n_support, tf_path=None):
    ...
    # metric function
    self.fc = nn.Sequential(nn.Linear(self.feat_dim, 128), nn.BatchNorm1d(128, track_running_stats=False)) if not (self.maml or self.maml_adain) else nn.Sequential(backbone.Linear_fw(self.feat_dim, 128), backbone.BatchNorm1d_fw(128, track_running_stats=False))
  ...
  def set_forward(self,x,is_feature=False):
    ...
    if is_feature:
      ...
      x = x.view(-1, *x.size()[2:])
      z = self.fc(self.feature(x))  # further encode the image features into a feature vector (128, )
      z = z.view(self.n_way, -1, z.size(1))   # reshape to  (num_way, num_support + num_querry, feature_dim)
...
class GnnNetLRP(GnnNet):
  ...
  def forward_gnn(self, zs):
    ...
    if self.training:
      ...
      nodes = torch.cat([torch.cat([zs[i] * relevance_weight[i], self.support_label], dim=2) for i in range(bs)], dim=0)
