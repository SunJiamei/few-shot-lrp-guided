...
class LFTNet(nn.Module):
  def __init__(self, params, tf_path=None, change_way=True):
    ...
    # backbone.FeatureWiseTransformationAdaIN2d_fw.feature_augment = True
    ...
    if params.method == 'protonet':
      ...
      if params.method == 'relationnetlrp':
        loss_type = 'LRPmse'
        model = relationnet.RelationNetLRP(feature_model, loss_type=loss_type, tf_path=params.tf_dir, **train_few_shot_params)
      else:
        model = relationnet.RelationNet(feature_model, loss_type=loss_type, tf_path=params.tf_dir, **train_few_shot_params)
    elif 'gnnnet' in params.method:
      gnnnet.GnnNet.maml=True
      gnnnet.GnnNetLRP.maml=True
      gnn.Gconv.maml=True
      ...
      if params.method == 'gnnnet':
        ...
        model = gnnnet.GnnNetLRP(model_dict[params.model], tf_path=params.tf_dir, **train_few_shot_params)
    else:
      raise ValueError('Unknown method')
    self.model = model
    # for module in model.relation_module.modules():
    #   if type(module) in [backbone.Linear_fw, backbone.Conv2d_fw]:
    #     print(type(module))
    #     print('bias', module.bias.fast)
    #     print(module.weight.fast)
    ...
    self.aux_loss_fn = nn.CrossEntropyLoss()   # use the encoded features to make the prediction
  ...
  def trainall_loop(self, epoch, ps_loader, pu_loader, aux_iter, total_it):
    ...
    for i, ((x, _, _), (x_new, _ , _)) in enumerate(zip(ps_loader, pu_loader)):
      ...
      ft_loss.backward()
      self.ft_optim.step()
      model_lr = self.model_optim.param_groups[0]['lr']
      ft_lr = self.ft_optim.param_groups[0]['lr']
      # loss
      avg_model_loss += model_loss.item()
      avg_ft_loss += ft_loss.item()

      if (i + 1) % print_freq == 0:
        # print('Epoch {:d}/{:d} | Batch {:d}/{:d} | LFT model_loss {:f}, ft_loss {:f}, model_lr {:f}, ft_lr {:f}'.format(\
        #     epoch + 1, self.total_epoch, i + 1, len(ps_loader), avg_model_loss/float(i+1), avg_ft_loss/float(i+1), model_lr, ft_lr))
        print('Epoch {:d}/{:d} | Batch {:d}/{:d} | LFT model_loss {:f}, ft_loss {:f}, model_lr {:f}, ft_lr {:f}'.format(\
            epoch + 1, self.total_epoch, i + 1, len(ps_loader), avg_model_loss/float(i+1), avg_ft_loss/float(i+1), model_lr, ft_lr))
  ...
  def train_loop(self, epoch, base_loader, total_it):
    ...
    # trainin loop
    for i, (x,_, _) in enumerate(base_loader):
      ...
      avg_model_loss += model_loss.item()
      if (i + 1) % print_freq == 0:
        print('Epoch {:d}/{:d} | Batch {:d}/{:d} | FT model_loss {:f}'.format(epoch + 1, self.total_epoch, i + 1, len(base_loader), avg_model_loss/float(i+1)))
      if (total_it + 1) % 10 == 0 and self.tf_writer is not None:
        self.tf_writer.add_scalar('FTNet/model_loss', model_loss.item(), total_it + 1)
      total_it += 1
    return total_it
