...
class BatchnormAdaIN2d(nn.BatchNorm2d):
  def __init__(self, num_features):
    super(BatchnormAdaIN2d, self).__init__(num_features, affine=True, track_running_stats=False)

  def forward(self, x, step=0):
    x_mean = torch.mean(x, dim=(-1, -2), keepdim=True)
    x_var = torch.var(x, dim=(-1, -2), keepdim=True)
    x_norm = (x - x_mean) * (torch.rsqrt(x_var + self.eps))
    out = self.weight.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand_as(x_mean) * x_norm + self.bias.unsqueeze(0).unsqueeze(
      -1).unsqueeze(-1).expand_as(x_mean)
    return out


class BatchnormAdaIN2d_fw(nn.BatchNorm2d):
  def __init__(self, num_features):
    super(BatchnormAdaIN2d_fw, self).__init__(num_features, affine=True, track_running_stats=False)
    self.weight.fast = None
    self.bias.fast = None
  def forward(self, x, step=0):
    if self.weight.fast is not None and self.bias.fast is not None:
    ...
    x_var = torch.var(x, dim=(-1, -2), keepdim=True)
    x_norm = (x - x_mean) * (torch.rsqrt(x_var + self.eps))
    out = weight.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand_as(x_mean) * x_norm + bias.unsqueeze(0).unsqueeze(
      -1).unsqueeze(-1).expand_as(x_mean)
    return out


class FeatureWiseTransformationAdaIN2d_fw(nn.BatchNorm2d):
  feature_augment = False
  def __init__(self, num_features, momentum=0.1, track_running_stats=False):
    super(FeatureWiseTransformationAdaIN2d_fw, self).__init__(num_features, momentum=momentum, track_running_stats=track_running_stats)
    self.weight.fast = None
    self.bias.fast = None
  ...
  def adaptive_instance_norm(self,x, weight, bias):
    '''this function implement the adaptive instance normalization
       size of x:  (N, C, H, W)
       we calculate the mean of the spatial dimension, to get the mean and var with shape (N, C)'''
    x_size = x.size()
    assert len(x_size) == 4
    # when the shape of x is (N, C, H, W)
    x_mean = torch.mean(x, dim=(-1, -2), keepdim=True)
    x_var = torch.var(x, dim=(-1, -2), keepdim=True)
    x_norm = (x - x_mean) * (torch.rsqrt(x_var+ self.eps))
    out = weight.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand_as(x_mean) * x_norm + bias.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand_as(x_mean)
    return out
  ...
  def forward(self, x, step=0):
    # print(self.running_mean.sum(), self.running_var.sum())
    if self.weight.fast is not None and self.bias.fast is not None:
      weight = self.weight.fast
      bias = self.bias.fast
    else:
    ...
    out = self.adaptive_instance_norm(x, weight, bias)
...
# --- Group normalization ----
class GroupNormalization(nn.BatchNorm2d):
  def __init__(self, num_features, num_group, affine=True):
    super(GroupNormalization, self).__init__(num_features, affine=affine, track_running_stats=False)
    self.num_group = num_group
    self.group_size = num_features // self.num_group

  def forward(self,x):
    N, C, H, W = x.size()
    x_grouped = x.view(N, self.num_group, self.group_size, H, W)
    x_mean = torch.mean(x_grouped, dim=(-1, -2), keepdim=True)
    x_var = torch.var(x_grouped, dim=(-1, -2), keepdim=True)
    x_norm = (x_grouped - x_mean) * (torch.rsqrt(x_var + self.eps))
    x_norm = x_norm.view(N, C, H, W)
    out = self.weight.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand_as(x_norm) * x_norm + self.bias.unsqueeze(0).unsqueeze(
      -1).unsqueeze(-1).expand_as(x_norm)
    return out

class GroupNormalization_fw(nn.BatchNorm2d):
  def __init__(self, num_features, group_size, affine=True):
    super(GroupNormalization_fw, self).__init__(num_features, affine=affine, track_running_stats=False)
    self.group_size = group_size
    self.num_group = num_features // self.group_size
    self.weight.fast = None
    self.bias.fast = None
  def forward(self,x):
    if self.weight.fast is not None and self.bias.fast is not None:
    ...
    x_norm = (x_grouped - x_mean) * (torch.rsqrt(x_var + self.eps))
    x_norm = x_norm.view(N, C, H, W)
    out = weight.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).expand_as(x_norm) * x_norm + bias.unsqueeze(0).unsqueeze(
      -1).unsqueeze(-1).expand_as(x_norm)
    return out
...
class FeatureWiseTransformation2d_fw(nn.BatchNorm2d):
  feature_augment = False
  def __init__(self, num_features, momentum=0.15, track_running_stats=True):
...
class ConvBlock(nn.Module):
  ...
  def __init__(self, indim, outdim, pool = True, padding = 1):
    ...
    elif self.maml_adain:
      self.C = Conv2d_fw(indim, outdim, 3, padding = padding)
      self.BN = FeatureWiseTransformationAdaIN2d_fw(outdim)
    else:
...
class SimpleBlock(nn.Module):
  ...
  def __init__(self, indim, outdim, half_res, leaky=False):
    ...
    if self.maml:
      ...
      self.relu1 = nn.ReLU(inplace=True) if not leaky else nn.LeakyReLU(0.2, inplace=True)
      self.C2 = Conv2d_fw(outdim, outdim,kernel_size=3, padding=1,bias=False)
      self.BN2 = FeatureWiseTransformationAdaIN2d_fw(outdim) # feature-wise transformation at the end of each residual block
    else:
    ...
    if indim!=outdim:
      if self.maml:
        ...
        self.shortcut = Conv2d_fw(indim, outdim, 1, 2 if half_res else 1, bias=False)
        self.BNshortcut = FeatureWiseTransformationAdaIN2d_fw(outdim)
      else:
  ...
  def forward(self, x):
    ...
    out = self.add(out, short_out)
    # out = out + short_out
    out = self.relu2(out)
    return out

# --- Simple ResNet Block adaptive normalization ---
class SimpleBlockAdaIN(nn.Module):
  maml = False
  maml_adain = False
  assert (maml and maml_adain) == False
  def __init__(self, indim, outdim, half_res, leaky=False):
    super(SimpleBlockAdaIN, self).__init__()
    self.indim = indim
    self.outdim = outdim
    if self.maml:
      self.C1 = Conv2d_fw(indim, outdim, kernel_size=3, stride=2 if half_res else 1, padding=1, bias=False)
      ...
      self.C1 = Conv2d_fw(indim, outdim, kernel_size=3, stride=2 if half_res else 1, padding=1, bias=False)
      self.BN1 = BatchnormAdaIN2d_fw(outdim)
      self.relu1 = nn.ReLU(inplace=True) if not leaky else nn.LeakyReLU(0.2, inplace=True)
      self.C2 = Conv2d_fw(outdim, outdim,kernel_size=3, padding=1,bias=False)
      self.BN2 = FeatureWiseTransformationAdaIN2d_fw(outdim) # feature-wise transformation at the end of each residual block
    else:
      ...
      self.relu1 = nn.ReLU(inplace=True) if not leaky else nn.LeakyReLU(0.2, inplace=True)
      self.C2 = nn.Conv2d(outdim, outdim,kernel_size=3, padding=1,bias=False)
      self.BN2 = BatchnormAdaIN2d_fw(outdim)
    self.parametrized_layers = [self.C1, self.C2, self.BN1, self.BN2]
    if indim!=outdim:
      if self.maml:
        self.shortcut = Conv2d_fw(indim, outdim, 1, 2 if half_res else 1, bias=False)
        ...
        self.shortcut = nn.Conv2d(indim, outdim, 1, 2 if half_res else 1, bias=False)
        self.BNshortcut = nn.BatchNorm2d(outdim, track_running_stats=False)
...
# --- Simple ResNet Block group normalization
class SimpleBlockGN(nn.Module):
  def __init__(self, indim, outdim, half_res, leaky=False):
    super(SimpleBlockGN, self).__init__()
    self.indim = indim
    self.outdim = outdim

    self.C1 = nn.Conv2d(indim, outdim, kernel_size=3, stride=2 if half_res else 1, padding=1, bias=False)
    self.BN1 = GroupNormalization(outdim, 32)
    self.relu1 = nn.ReLU(inplace=True) if not leaky else nn.LeakyReLU(0.2, inplace=True)
    self.C2 = nn.Conv2d(outdim, outdim,kernel_size=3, padding=1,bias=False)
    self.BN2 = GroupNormalization(outdim, 32)
    self.parametrized_layers = [self.C1, self.C2, self.BN1, self.BN2]
    if indim!=outdim:

      self.shortcut = nn.Conv2d(indim, outdim, 1, 2 if half_res else 1, bias=False)
      self.BNshortcut = GroupNormalization(outdim,32)
...
class ResNetGN(nn.Module):
  ...
  def __init__(self,block,list_of_num_layers, list_of_out_dims, flatten=True, leakyrelu=False):
    ...
    if self.maml:
      conv1 = Conv2d_fw(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
      bn1 = GroupNormalization_fw(64,32)
    elif self.maml_adain:
      conv1 = Conv2d_fw(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
      bn1 = GroupNormalization_fw(64,32)
    else:
      conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
      bn1 = GroupNormalization(64,32)
...
def ResNet10AdaIN(flatten=True, leakyrelu=False):
    return ResNet(SimpleBlockAdaIN, [1,1,1,1],[64,128,256,512], flatten, leakyrelu)
def ResNet10GN(flatten=True, leakyrelu=False):
    return ResNetGN(SimpleBlockGN, [1,1,1,1],[64,128,256,512], flatten, leakyrelu)
def ResNet18(flatten=True, leakyrelu=False):
    return ResNet(SimpleBlock, [2,2,2,2],[64,128,256,512], flatten, leakyrelu)
def ResNet18GN(flatten=True, leakyrelu=False):
  return ResNetGN(SimpleBlockGN, [2, 2, 2, 2], [64, 128, 256, 512], flatten, leakyrelu)
...
model_dict = dict(Conv4 = Conv4,
                  ...
                  ResNet18 = ResNet18,
                  ResNet34 = ResNet34,
                  Resnet10AdaIN = ResNet10AdaIN,
                  ResNet18GN = ResNet18GN,
                  ResNet10GN = ResNet10GN)
