...
def feature_evaluation_trans(cl_data_file, model, n_way = 5, n_support = 5, n_query = 15, candicate_num=35):
  class_list = cl_data_file.keys()
  select_class = random.sample(class_list,n_way)
  z_all  = []
  for cl in select_class:
  ...
  scores = model.set_forward_transductive(z_all, is_feature=True)
...
if __name__ == '__main__':
  ...
  # params.name = 'RelationNetcheckpoints/relationnetlrpadd0.5'
  params.name = 'RelationNetcheckpoints/relationnet'
  # params.name = 'RelationNetcheckpoints/FTrelationnet_cars'
  # params.name = 'RelationNetcheckpoints/multirelationnet_cub'
  params.method = 'relationnet'
  params.testset = 'cub'
  params.n_shot = 5
  print(params.transductive)
  print('Testing! {} shots on {} dataset with {} epochs of {}({})'.format(params.n_shot, params.testset, params.save_epoch, params.name, params.method))
  remove_featurefile = False
  ...
  if params.method in ['relationnet', 'relationnet_softmax', 'relationnetlrp']:
  ...
  if params.save_epoch != -1:
    featurefile = os.path.join( checkpoint_dir.replace("checkpoints","features"), params.testset + '_' + split + "_" + str(params.save_epoch)+ ".hdf5")
  else:
    featurefile = os.path.join( checkpoint_dir.replace("checkpoints","features"), params.testset + '_' + split + ".hdf5")
  ...
  if params.method == 'protonet':
    ...
    model = GnnNetLRP( model_dict[params.model], **few_shot_params)
  ...
  for i in range(iter_num):
    if params.transductive:
      acc = feature_evaluation_trans(cl_data_file, model, n_query=15, candicate_num=35, **few_shot_params)
    else:
